...
class BooleanIRHandler(object):
    ...
    def handle_query(self, query, indexterms_dict, docs_dict):
        ...
        logging.debug('replaced index terms in boolean query: {}'.format(parse_result))
        ...
        logging.debug('calculated and-clauses: {}'.format(intersect_result))
        union_result = BooleanIR().union_literals(intersect_result, universe)
        logging.debug('calculated or-clause: {}'.format(union_result))
